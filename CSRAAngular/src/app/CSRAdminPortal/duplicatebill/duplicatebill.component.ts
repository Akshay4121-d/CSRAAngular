...
@Component({
  selector: 'app-duplicatebill',
  templateUrl: './duplicatebill.component.html',
  styleUrls: ['./duplicatebill.component.css']
})
export class DuplicatebillComponent implements OnInit {
  ...
  validatePolicyInfoBySearch() {
    ...
    let l_requestType = this.common.encrypt('duplicatebill');
  ...
  {    
    if (validationMsg == "Success") {
      ...
      // let url = '/policyinfosearch?search=' + encodeURIComponent(l_Policyno.trim()) + '%7C' + encodeURIComponent(l_ZipCode.trim());
       let url = '/duplicatebillcontact?pno='+ encodeURIComponent(l_Policyno.trim());
      ...
      this.norecords=true;
      this.l_message="A duplicate bill is unavailable for the current coverage term as our records show this policy has been paid.";
